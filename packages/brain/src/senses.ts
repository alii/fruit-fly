...
type Tail<S extends string, H extends string> = S extends `${H}/${infer T}`
  ? T
  : never;
type Prefix<S extends string> = S extends `${infer H}/${infer T}`
  ? H | `${H}/${Prefix<T>}`
  : never;
...
export type SenseTree<S extends string = ChannelName> = Sense & {
  readonly [H in Head<S>]: [Tail<S, H>] extends [never]
    ? Sense
    : SenseTree<Tail<S, H>>;
};
...
export function senses(neurons: Neurons, opts: SensesOptions = {}): Senses {
  ...
  const at = (path: SensePath): Sense =>
    path
      .split("/")
      .reduce<Sense>(
        (n, seg) => (n as unknown as Record<string, Sense>)[seg]!,
        root,
      );
...
function node(
  ...
  const kids = Object.fromEntries(
    [...children].map(([seg, cs]) => [
      seg,
      node(path === "" ? seg : `${path}/${seg}`, cs, maxHz),
    ]),
  );
  return {
    ...kids,
...
function mechanosensory(l: Labels): string {
  ...
  if (l.type.startsWith("BM_") || l.type.startsWith("TPMN"))
    return `head/mouth/${l.side}`;
...
function proprioceptive(l: Labels): string {
  ...
  if (l.leg)
    return `leg/${l.leg}/${l.side}/${LEG_PROPRIO_BY_SUBCLASS[l.sub] ?? "other"}`;
...
function unknownSensory(l: Labels): string {
  if (l.sub === "abdomen" || l.nerve.startsWith("AbN"))
    return `abdomen/${l.side}/touch`;
