...
export class Eye {
	...
	sample(frame: Frame): Float32Array {
		const scale =
			frame.data instanceof Uint8Array || frame.data instanceof Uint8ClampedArray ? 1 / 255 : 1;
	...
	see(brain: Lif, frame: Frame): void {
		...
		this.on = Float32Array.from(lum, (l, k) => Math.max(this.on[k]! * decay, l - this.prev[k]!));
		this.off = Float32Array.from(lum, (l, k) => Math.max(this.off[k]! * decay, this.prev[k]! - l));
		this.prev = lum;
