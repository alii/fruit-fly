...
const frame = (radius: number): Frame => ({
	...
	data: Float32Array.from({length: W * H}, (_, p) =>
		((p % W) - W / 2) ** 2 + (Math.floor(p / W) - H / 2) ** 2 < radius ** 2 ? 0 : 1,
	),
});
/** disc radius at time t: blank, then growing over 300 ms, then held */
const radiusAt = (t: number) => (t < 100 ? 0 : t < 400 ? ((t - 100) / 300) * (W / 2) : W / 2);
...
const world: World = {
	...
	act: (t, spikes, _b, n) =>
		spikes
			.filter(i => gf.has(i))
			.forEach(i => console.log(`t=${t.toFixed(1)}ms  ${n.describe(i)} FIRED`)),
};
runWorld(brain, neurons, world, {ms: 500, senseEveryMs: 5});

const sum = (ids: Uint32Array) => ids.reduce((acc, i) => acc + brain.counts[i]!, 0);
